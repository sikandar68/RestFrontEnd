...
const ServiceChargesWindow: React.FC<EposCheckOutFormProps> = ({
  orderData,
  onUpdateOrderData,
  ...
  const handleSaveClick = () => {
    ...
    if (type === 'percentage') {
      ...
      if (!isNaN(numericValue)) {
        serviceCharges = (numericValue / 100) * total;
      } else {
      ...
      const numericValue = parseFloat(value);
      if (!isNaN(numericValue)) {
        serviceCharges = numericValue;
      } else {
        console.error('Invalid Service Charges value:', value);
      }
    }
    onUpdateOrderData({
      ...orderData,
    ...
    setServiceChargesWindowOpen(false);
  };
...
export default ServiceChargesWindow;
